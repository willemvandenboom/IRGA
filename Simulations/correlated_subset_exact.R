...
for(i in 1:n.q) for(j in 1:n.rep) {
  ...
  PIP <- PIP_exact(y, X = A, lambda, psi, Sigma = sigma.sq * diag(n))[1:2]
...
pdf("simulation_correlated_subset_exact.pdf", width = 5.5, height = 2)
...
points(EP_error_median, col = 'blue', pch = mid.symbol)
points(EP_error_min, col = 'blue', pch = q.symbol)
points(EP_error_max, col = 'blue', pch = q.symbol)
for(i in 1:n.q) lines(x = rep(i, 2), y = c(EP_error_min[i], EP_error_max[i]), col = 'blue')

points(VB_error_median, col = 'red', pch = mid.symbol)
points(VB_error_min, col = 'red', pch = q.symbol)
points(VB_error_max, col = 'red', pch = q.symbol)
for(i in 1:n.q) lines(x = rep(i, 2), y = c(VB_error_min[i], VB_error_max[i]), col = 'red')
...
pdf("simulation_correlated_subset_exact_time.pdf", width = 5.5, height = 2)
par(mfrow = c(1,1), bty = 'l',
    cex = .5)
